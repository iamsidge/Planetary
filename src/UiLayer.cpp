...
void UiLayer::setup( PlaylistChooserRef playlistChooser, 
                     ...
                     const gl::TextureRef &uiButtonsTex, 
                     const bool &showSettings, 
                     const vec2 interfaceSize )
{
...
void UiLayer::updateLayout( vec2 interfaceSize )
{    
...
bool UiLayer::touchMoved( TouchEvent::Touch touch )
{
	...
	if( mIsPanelTabTouched ){
        ...
        vec2 newPos = touchPos + mPanelTabTouchOffset;
...
{	
	...
	gl::drawLine( vec2( 0.0f, 0.0f ), vec2( mPanelTabRect.x1 + 22.0f, 0.0f ) );
...
bool UiLayer::hitTest( vec2 globalPos ) 
{
    if (mVisible) {
        vec2 pos = globalToLocal( globalPos );
